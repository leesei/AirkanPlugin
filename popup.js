...
chrome.windows.getCurrent(function(wnd) {
  chrome.tabs.getSelected(wnd.id, function(tab) {
    ...
    setInterval(function() {
      if (isRefreshDeviceList && refreshCount < 5) {
        refreshCount = refreshCount + 1;
        chrome.browserAction.setBadgeText({
          text: BG.device_list.length.toString(),
          tabId: tab.id
        }); //数字提示设备个数
        showDeviceList(BG.device_list, BG.mediaurls[id]);
...
function showDeviceList(data, mediaurl) {
  ...
  btnRefresh.addEventListener('click', function() {
    ...
    if (BG.device_list[BG.currenDeviceIndex] !== undefined) {
      console.log("Set current Device IP is " + BG.device_list[BG.currenDeviceIndex].ip);
    }
  }, false);
  btnRefresh.addEventListener('mouseover', function() {
    var para = this.className = "refresh1";
  });
  btnRefresh.addEventListener('mouseout', function() {
    var para = this.className = "refresh";
  });
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    if (mediaurl !== undefined) {
      newrow.addEventListener('click', function() {
        selectDevice(this.name);
        var ipText = document.getElementById("ip" + this.name);
        if (ipText.innerHTML != "播放中") {
          onClickVideoPlay(mediaurl[0].name, mediaurl[0].url, BG.device_list[this.name].ip);
          ipText.innerHTML = "播放中";
        } else {
          onClickVideoStop(mediaurl[0].name, mediaurl[0].url, BG.device_list[this.name].ip);
          ipText.innerHTML = BG.device_list[this.name].ip.replace(/http:\/\//, '').replace(/:6095/,'');
        }
      });
    }
    ...
    item.addEventListener('mouseover', function() {
      var para = this.className = "testOver";
    });
    item.addEventListener('mouseout', function() {
      var para = this.className = "testNormal";
...
function filltable(data, isMedia) {
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    if (isMedia) {
      ...
      newcell.firstChild.addEventListener('click', function() {
        onClickVideoPlay(this.name, this.alt);
      });
...
function onClickVideoPlay(name, media_url, ip) {
  xmlHttp = createHttpRequest();
  var url2 = ip + "/video/action=play&clientname=PC&url=" + encodeURIComponent(media_url) + "&title=" + encodeURIComponent(name);
  xmlHttp.open("get", url2, true);
  xmlHttp.setRequestHeader("Content-Type", "text/json");
  xmlHttp.send();
  console.log("Send " + url2);
...
function onClickVideoStop(name, media_url, ip) {
  xmlHttp = createHttpRequest();
  var url2 = ip + "/controller?action=keyEvent&keyCode=back";
  xmlHttp.open("get", url2, true);
  xmlHttp.setRequestHeader("Content-Type", "text/json");
  xmlHttp.send();
  console.log("Send " + url2);
